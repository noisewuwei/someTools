...
@interface YM_UpwardScoreView ()
{
    BOOL _scoreInTeamCenter;
}
@property (strong, nonatomic) YM_UpwardBifurcationView * bifurcationView;
...
@implementation YM_UpwardScoreView
...
- (void)layoutSubviews {
    ...
    [_bifurcationView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(_leftTeamView.mas_bottom).offset(5);
...
#pragma mark - 懒加载
- (YM_UpwardBifurcationView *)bifurcationView {
    if (!_bifurcationView) {
        _bifurcationView = [YM_UpwardBifurcationView new];
    }
