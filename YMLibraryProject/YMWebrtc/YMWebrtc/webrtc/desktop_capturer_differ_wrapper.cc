...
#include "desktop_capturer_differ_wrapper.h"
...
//  RTC_DCHECK_GT(last_block_width, 0);
  ...
  for (int x = 0; x < block_count; x++) {
    if (BlockDifference(old_buffer, new_buffer, height, stride)) {
      ...
      // The block on the left is the last dirty block in a continuous dirty area.
      output->AddRect(
  ...
  if (last_block_width < kBlockSize) {
    ...
    last_block_diff = PartialBlockDifference(old_buffer, new_buffer, last_block_width, height, stride);
  } else {
  ...
  if (last_block_diff) {
      ...
      output->AddRect(DesktopRect::MakeLTRB(first_dirty_x_block * kBlockSize + left, top, right, bottom));
  } else if (first_dirty_x_block != -1) {
      output->AddRect(DesktopRect::MakeLTRB(first_dirty_x_block * kBlockSize + left, top,
...
void DesktopCapturerDifferWrapper::OnCaptureResult( Result result, std::unique_ptr<DesktopFrame> input_frame) {
...
//    }
  ...
  if (last_frame_) {
    ...
    for (DesktopRegion::Iterator it(hints); !it.IsAtEnd(); it.Advance()) {
        // 比较帧数据的差异
        CompareFrames(*last_frame_, *frame, it.rect(), frame->mutable_updated_region());
    }
  } else {
  ...
  if (result == webrtc::DesktopCapturer::Result::SUCCESS && frame && !frame->updated_region().is_empty()) {
      callback_->OnCaptureResult(result, std::move(frame));
  } else {
//      std::cout << "[" << "没有差异" << "]" << std::endl;
  }
}
