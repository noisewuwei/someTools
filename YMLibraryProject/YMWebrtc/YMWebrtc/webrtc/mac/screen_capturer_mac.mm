...
#include "mac/desktop_frame_provider.h"
#include "mac/window_list_utils.h"
...
//    MacDesktopConfiguration new_config_1 = desktop_config_monitor_->desktop_configuration();
//      if (!desktop_config_.Equals(new_config_1)) {
//        desktop_config_ = new_config_1;
...
//        continue;
//      }
//      frame_source->mutable_updated_region()->AddRect(DesktopRect::MakeSize(frame_source->size()));
//      if (current_display_) {
//        const MacDisplayConfiguration* config =
//            desktop_config_.FindDisplayConfigurationById(current_display_);
//        if (config) {
//          frame_source->set_top_left(
//              config->bounds.top_left().subtract(desktop_config_.bounds.top_left()));
//        }
//      }
//
//      callback_->OnCaptureResult(Result::SUCCESS, std::move(frame_source));
//
////        printf("set_capture_time_ms:%lld \n", (rtc::TimeNanos() - capture_start_time_nanos) /
...
//      RTC_LOG(LS_ERROR) << "Failed to register refresh and move handlers.";
  ...
  // With IOSurface method, the zoomed screen contents will be captured.
    //当屏幕放大/缩小时，OSX当前只更新矩形部分
    //显示在屏幕上，与当前屏幕左上角的相对位置。
    //当我们将脏区域复制到捕获的对象时，这将导致问题
    //形象。因此，我们要使整个屏幕无效以复制所有屏幕内容。
    //使用CGI方法，缩放将被忽略，整个屏幕的内容
    //会像以前一样被抓获。
    //使用IOSurface方法，将捕获缩放的屏幕内容。
