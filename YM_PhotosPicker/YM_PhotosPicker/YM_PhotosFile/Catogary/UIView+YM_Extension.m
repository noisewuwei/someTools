...
- (void)showImageHUDText:(NSString *)text {
    CGFloat hudW = [YM_PhotoTools getTextWidth:text height:15 fontSize:14];
    if (hudW > self.frame.size.width - 60) {
        hudW = self.frame.size.width - 60;
    }
    CGFloat hudH = [YM_PhotoTools getTextHeight:text width:hudW fontSize:14];
    if (hudW < 100) {
        hudW = 100;
    }
...
- (void)setup {
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[YM_PhotoTools ym_imageNamed:self.imageName]];
    ...
    CGFloat labelH = [YM_PhotoTools getTextHeight:self.text width:labelW fontSize:14];
