...
@implementation YM_CollectionView
...
- (void)gestureRecognizerBegan:(UILongPressGestureRecognizer *)longPgr
{
    ...
    YM_PhotoSubViewCell *cell = (YM_PhotoSubViewCell *)[self cellForItemAtIndexPath:self.originalIndexPath];
...
- (void)handelItemInSpace {
    ...
    for (NSIndexPath *indexPath in totalArray) {
        YM_PhotoSubViewCell *sectionLastCell = (YM_PhotoSubViewCell *)[self cellForItemAtIndexPath:indexPath];
        ...
        if (sectionLastCell.model.type == YM_PhotoModelMediaType_Camera) {
            continue;
        }
...
- (void)moveCell{
    ...
    for (YM_PhotoSubViewCell *cell in [self visibleCells]) {
