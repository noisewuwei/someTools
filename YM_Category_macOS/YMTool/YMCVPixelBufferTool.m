...
@interface YMCVPixelBufferTool ()

@end

@implementation YMCVPixelBufferTool
...
+ (CVReturn)ymCreateResolvedAttributesDictionary:(CFAllocatorRef)allocator
    ...
    return CVPixelBufferCreateResolvedAttributesDictionary(allocator, attributes, resolvedDictionaryOut);
}
...
+ (size_t)ymGetBytesPerRowOfPlane:(CVPixelBufferRef)pixelBuffer planeIndex:(size_t)planeIndex {
    return CVPixelBufferGetBytesPerRowOfPlane(pixelBuffer, planeIndex);
}
