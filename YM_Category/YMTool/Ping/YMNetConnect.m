...
- (void)runWithHostAddress:(NSString *)hostAddress port:(int)port
{
    ...
    _sumTime = 0;
    _resultLog = @"";
//    if (self.delegate && [self.delegate respondsToSelector:@selector(ymConnectLog:)]) {
//        [self.delegate
//            ymConnectLog:[NSString stringWithFormat:@"connect to host %@ ...", _hostAddress]];
//    }
                                     ...
                                     CFDataRef address, const void *data, void *info)
{
    if (data != NULL) {
        printf("connect");
        YMNetConnect *con = (__bridge_transfer YMNetConnect *)info;
...
- (void)readStream:(BOOL)success {
    ...
    if (_connectCount < MAXCOUNT_CONNECT) {
        ...
        // 返回日志
        if ([self.delegate respondsToSelector:@selector(ymConnectLog:)]) {
            [self.delegate ymConnectLog:_resultLog];
        }
        
        // 返回平均值
        if ([self.delegate respondsToSelector:@selector(ymConnectAverage:)]) {
            [self.delegate ymConnectAverage:(long)(_sumTime / 4)];
        }
        
        if (self.delegate && [self.delegate respondsToSelector:@selector(ymConnectDidEnd:)]) {
            [self.delegate ymConnectDidEnd:_isExistSuccess];
        }
