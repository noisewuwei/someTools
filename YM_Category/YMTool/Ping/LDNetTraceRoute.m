...
- (Boolean)doTraceRoute:(NSString *)host {
    ...
    NSArray *serverDNSs = [YMNetworkTool ymDNSWithHost:host];
    if (!serverDNSs || serverDNSs.count <= 0) {
        if (_delegate != nil) {
            [_delegate ymTraceLog:@"TraceRoute>>> Could not get host address"];
            [_delegate ymTraceRouteDidEnd];
        }
        return false;
    }
    ...
    if ((recv_sock = socket(destination->sa_family, SOCK_DGRAM, isIPV6?IPPROTO_ICMPV6:IPPROTO_ICMP)) < 0) {
        if (_delegate != nil) {
            [_delegate ymTraceLog:@"TraceRoute>>> Could not create recv socket"];
            [_delegate ymTraceRouteDidEnd];
        }
        return false;
    }
    ...
    if ((send_sock = socket(destination->sa_family, SOCK_DGRAM, 0)) < 0) {
        if (_delegate != nil) {
            [_delegate ymTraceLog:@"TraceRoute>>> Could not create xmit socket"];
            [_delegate ymTraceRouteDidEnd];
        }
        return false;
    }
    ...
    while (ttl <= maxTTL) {
        ...
        if ((isIPV6? setsockopt(send_sock,IPPROTO_IPV6, IPV6_UNICAST_HOPS, &ttl, sizeof(ttl)):setsockopt(send_sock, IPPROTO_IP, IP_TTL, &ttl, sizeof(ttl))) < 0) {
            error = true;
            if (_delegate != nil) {
                [_delegate ymTraceLog:@"TraceRoute>>> setsockopt failled"];
            }
        }
        ...
        if (icmp) {
            [self.delegate ymTraceLog:traceTTLLog];
            if (_sumDelta > -1) {
                [self.delegate ymTraceAverage:(long)(_sumDelta / count)
                                         host:hostAddress];
                _sumDelta = -1;
                count = 0;
            }
            ...
            if (timeoutTTL >= 4) {
                ...
                [self.delegate ymTraceLog:[NSString stringWithFormat:@"%d\t********\t", ttl]];
            }
        }
