...
- (void)showTextWithType:(kTableViewType)type {
    ...
    NSString * imageName = @"";
    switch (type) {
        case kTableViewType_NoShow:
            text = @"";
            break;
        case kTableViewType_NoData:
            text = @"暂无数据";
            break;
        case kTableViewType_TouchReload:
            imageName = @"iPublic_TouchRefresh";
            text = @"轻触加载";
            break;
        case kTableViewType_NetworkError:
            text = @"请检查网络";
            break;
        default: break;
    }
...
- (void)showViewWithText:(NSString *)text imageName:(NSString *)imageName {
    ...
    if (image) {
        ...
        frame.origin.y = textLabel.frame.origin.y - 10 - frame.size.height;
