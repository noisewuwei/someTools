...
- (void)textFieldDidBeginEditing {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        _imageView.left = 5;
        _textField.left = _imageView.right + _spaceWidth;
        _textField.width = self.width - _textField.left;
    }];
}
...
- (void)textFieldDidEndEditing {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        _imageView.right = self.centerX - _spaceWidth;
        _textField.left = self.centerX;
        _textField.width = self.width - _textField.left;
    }];
}
...
- (UIImageView *)imageView {
    if (!_imageView) {
        ...
        imageView.frame = CGRectMake(self.centerX - imageSize.width - _spaceWidth, 0, imageSize.width, imageSize.height);
...
- (UITextField *)textField {
    if (!_textField) {
        ...
        textField.frame = CGRectMake(self.centerX, 0, self.width - self.centerX, self.height);
...
- (void)layoutView {
    ...
    cancelBtn.frame = CGRectMake(self.width - _cancelBtnWidth, 0, _cancelBtnWidth, self.height);
...
- (void)startEdit {
    [UIView animateWithDuration:0.5 animations:^{
        _searchBar.width = self.width - _cancelBtnWidth;
    }];
}
...
- (YM_SeachBar *)searchBar {
    if (!_searchBar) {
        YM_SeachBar * view =
        [[YM_SeachBar alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height)
                                   content:_content
        ...
        view.contentDidChangeBlock = _contentDidChangeBlock;
